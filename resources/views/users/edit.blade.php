    ...
    <div class="card-body">
        ...
        <form method="POST" action="{{ route('users.update', $user) }}">
            ...
            <div class="form-group">
                ...
                <select name="user_role_template" id="user_role_template" class="form-control" {{ $user->is_platform_admin ? 'disabled' : '' }}>
                    @foreach($userRoleTemplates as $templateKey => $templateLabel)
                        <option value="{{ $templateKey }}" {{ old('user_role_template', $selectedUserRoleTemplate) === $templateKey ? 'selected' : '' }}>
                            {{ $templateLabel }}
                        </option>
                    @endforeach
                </select>
                @if($user->is_platform_admin)
                    <input type="hidden" name="user_role_template" value="admin">
                @endif
...
<script>
    (function () {
        ...
        function applyRoleTemplate() {
            if (!userRoleTemplate || !adminCheckbox || isPlatformAdmin) return;
